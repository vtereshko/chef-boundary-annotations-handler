...
        event_url = create_event(gist_id, tags, "Chef","INFO","OK","Chef Event Notification")
      end
    end
...
      event_url = create_event(gist_id, tags, "Chef","WARN","OPEN","Chef Exception")
    end
...
  def create_event(gist_id, tags, type, level,status,title)
...
    headers = {"Authorization" => "Basic #{auth}", "Content-Type" => "application/json"}
    ...
    event = {
	 :source => "Chef",
      ...
      :tags => tags,
      :title => "Chef event notification" ,
      :createdAt => run_status.start_time.to_i
    }

    event_json = event.to_json

    uri = URI("https://api.boundary.com/events/#{@boundary_orgid}")
